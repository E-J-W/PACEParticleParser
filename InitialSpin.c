...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("PACE_InitialSpin particles_file\n");
      printf("\nGenerates a histogram of initial compund nucleus spins for all events in the input\n.particles file generated by PACE4.\n");
      exit(-1);
    }
  ...
  if(h!=NULL)
    delete h;
  h=new TH1I("InitialSpin","InitialSpin",50,0,50);
  if(c!=NULL)
    delete c;
  c = new TCanvas("InitialSpin","InitialSpin",10,10, 700, 500);
  ...
  if(fgets(str[0],132,inp)!=NULL)
	...
	{
          ji = atof(str[8]); //get initial spin of the compund nucleus
          h->Fill(ji);//increment the histogram
        }
    else
      {
